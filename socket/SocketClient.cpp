...
bool SocketClient::connectServer(const char* serverIP, unsigned short port)
{
	...
	CCLOG("client connect the server");
	if (ret < 0)
	{
		_socektClient = 0;
...
void SocketClient::recvMessage()
{
	...
	_mutex.lock();
	this->closeConnect(_socektClient);
	CCLOG("%s", "close the socket client");
